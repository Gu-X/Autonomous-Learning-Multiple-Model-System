...
Lambda0=numpy.exp(-1/8) # Threshold for adding new rules. The lower the value is, the larger the rule base will be. (Changeable)
...
def Centre_lambda_calculator(ModeNumber,data,Centre,LocalX,Support,GlobalMean,GlobalX):
    ...
    for ii in range(0,ModeNumber):
        CentreDensity[ii]=1/(numpy.sum(numpy.power((Centre[ii,:]-GlobalMean),2))+GDelta+1e-10)
    ...
    for ii in range(0,ModeNumber):
        ...
        if (AA+BB)==0:
            ...
            LocalDensity[ii]=numpy.exp(-0.5*BB/AA)
...
def Global_density_calculator(data,GlobalMean,GlobalX):
    ...
    GlobalMean=numpy.matrix(GlobalMean)
    GDelta=GlobalX-numpy.sum(numpy.power(GlobalMean,2))
    DataDensity=1/(numpy.sum(numpy.power(data-GlobalMean,2))+GDelta+1e-10)
    return DataDensity
